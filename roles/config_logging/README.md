...
Runs on every host (sibling to `config_hardening`) and forwards logs to the central
...
`config_logging_extra_labels`) so you can filter and correlate in Grafana.

Network & safety
----------------

- Ships to `config_logging_loki_host` (`mollymauk.oob.frankenas.lan`) on `:3100`
  over **OOB** — log traffic never touches WAN, and it works through the
  `config_vpn_client` kill switch (the mgmt CIDR bypasses the tunnel).
- The log server sets `config_logging_enabled: false` for itself to avoid a
...
- `config_logging_enabled` (default `true`) — master switch.
- `config_logging_loki_host` / `config_logging_loki_port` — central Loki endpoint (OOB).
- `log_sources_enabled` / `log_sources_overrides` — per-host sources.
- `config_logging_extra_labels` — extra static labels on every stream.
