...
- **`provision_proxmox`** builds the LXC (s8 OS template) and applies the `nott` +
  `config` bind mounts.
- **`service_loki`** (this role) lays out storage dirs and renders
  `loki-config.yaml` + Grafana datasource provisioning.
- **`service_docker` role** deploys the `loki` and `grafana` containers from
  `docker_containers_catalog` (group `observability`).
- **`config_logging`** runs on every *other* host to ship logs here.

Key facts
---------

- **Retention: 180 days** (`service_loki_retention_period: 4320h`), enforced by
...
- **Loki is unauthenticated.** `auth_enabled: false` is *multi-tenancy*, not
  ...
  **OOB** network only and (b) a `config_firewall` profile restricting `:3100` to the
  OOB CIDR. **Never expose it to WAN.**
- **Grafana** has its own login; set `grafana_admin_password` from
...
- `service_loki_data_root` (`/mnt/nott/loki`) — heavy storage on the Nott bind.
- `service_loki_retention_period` (`4320h` = 180d), `service_loki_retention_enabled`.
- `service_loki_loki_http_port` (`3100`), `grafana_port` (`3000`).
- `grafana_admin_password` — **set from vault**.
