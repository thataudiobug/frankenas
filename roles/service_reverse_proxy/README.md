...
Files dropped (under `{{ service_reverse_proxy_npm_data_path }}/nginx/custom/`, visible inside
...
- `service_reverse_proxy_npm_data_path` (`/mnt/config/npm/data`) — NPM's bind-mounted `/data`.
- `service_reverse_proxy_authelia_internal_url` — Authelia's OOB endpoint NPM calls.
- `service_reverse_proxy_authelia_portal_url` — public portal redirect target.
- `service_reverse_proxy_authelia_enabled` — master toggle.
