...
2. the management subnet (`vpn_mgmt_cidr`) — so SSH/Ansible keep working,
3. the resolved VPN endpoint IP(s) on the physical NIC — the bootstrap
   exception OpenVPN needs to build the tunnel,
4. anything out the tunnel device (`vpn_tun_dev`).
...
`group_vars/vpn_client/vpn_server_catalog.yml`:
...
- `vpn_mgmt_cidr` (default `10.128.0.0/9`) — management subnet allowed
  through the kill switch. Get this right or risk SSH lockout.
- `vpn_endpoint_rules_override` — pin firewall bootstrap rules manually as
  `[{ip, port, proto}, ...]` when control-node DNS differs from the target
  (geo-DNS) or DNS is unavailable.
- `vpn_dns_servers`, `vpn_disable_ipv6`, `vpn_tun_dev`, `vpn_verify_strict`.
...
`vpn_verify_strict: true` (default) any failure aborts the play.

The same checks run stand-alone via `plays/vpn_killswitch_verify.yml` for
ad-hoc or scheduled leak testing without re-applying configuration.

## Lockout safety

`vpn_mgmt_cidr` is allowed both directions before any default-deny takes
