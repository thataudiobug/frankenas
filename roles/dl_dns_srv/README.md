...
> Naming note: the role is `dl_dns_srv` even though it serves DHCP **and**
> DNS. DNS (FQDN `ansible_host`) is the user-facing goal; DHCP is the
> mechanism that feeds it. The name matches the `dl_*` droplet-role stem and
> reads as "the DNS server droplet."
...
- **Existing logic (unchanged):** `pve_dl`'s `build_networks.yml` DHCP support
